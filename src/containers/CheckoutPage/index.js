...
const CheckoutStep = (props) => {
    return (
          <div className={`checkoutStep ${props.pointer && 'pointer'}`} onClick={props.onClick}>
        ...
        <div className="flexRow sb addressinfo">
          {!adr.edit ? (
            ...
            <AddressForm
              ...
              initialData={adr}
              onCancel={() => closeAddressEditForm(adr)}
            />
          )}
        </div>
    </div>
...
export default function CheckoutPage(props){
    ...
    const closeAddressEditForm = (addr) => {
        const updatedAddress = address.map((adr) =>
        ...
        <Layout>
            <div className="cartContainer" style={{ alignItems: 'flex-start'}}>
                <div className="checkoutContainer">
                            ...
                            {confirmAddress?  (
                                ...
                                address.map((adr, index) => (
                                  <Address key={index}
                                    ...
                                    closeAddressEditForm={closeAddressEditForm}
                                    adr={adr}
                                  />
                                  )))}
                                  </>
                  ...
                  <CheckoutStep 
                      active={auth.authenticate && confirmAddress && orderSummary}
                      stepNumber={'3'}
                      title={'ORDER SUMMARY'}
                      body={
                        orderSummary ?
                        <CartPage onlyCartItems={true} /> :
                        orderConfirmation ? <div className='stepCompleted'>{Object.keys(cart.cartItems).length} items</div> : null
                      }
                  />
                  {
                    orderSummary &&
