    ...
    return async dispatch => {
        try {
            ...
            if (res.status === 200) {
                ...
                dispatch({type: userConstants.GET_USER_ADDRESS_SUCCESS,
                            payload: {address}})
            } else {
                const {error} = res.data
                dispatch({type: userConstants.GET_USER_ADDRESS_FAILURE,
                            payload: {error}})
            }
        } catch (error) {
            dispatch({type: userConstants.GET_USER_ADDRESS_FAILURE,
...
export const getOrder = (payload) => {
    return async (dispatch) => {
        try {
            ...
            if(res.status === 200){
                const {order} = res.data
                dispatch({type: userConstants.GET_USER_ORDER_DETAILS_SUCCESS,
                payload: {order}})
            } else {
                const {error} = res.data
                dispatch({type: userConstants.GET_USER_ORDER_DETAILS_FAILURE,
                payload: {error}})
            } 
        } catch(error){
            dispatch({type: userConstants.GET_USER_ORDER_DETAILS_FAILURE,
