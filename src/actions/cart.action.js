...
export const addToCart = (product, newQty=1) => {
    return async dispatch => {
        const { cart: {cartItems}, auth } = await store.getState()
        console.log(cartItems, cartItems[product._id], product._id)
        const qty = await cartItems[product._id] ? parseInt(cartItems[product._id].qty + newQty) : 1
    ...
    return async(dispatch) => {
        try{
            ...
            if(res.status === 202) {
                ...
                dispatch({type: cartConstants.REMOVE_FROM_CART_FAILURE, 
                    payload: {error}})
            } 
        }catch (error){
            console.log(error)
